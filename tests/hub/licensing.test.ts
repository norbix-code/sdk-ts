...
import { LicensingModule } from '../../src/hub/licensing.js';
 ...
 *
 * Tests for hub.licensing (3 endpoints).
 *
 ...
 */
describe('hub.licensing', () => {
  it('module exposes 3 method(s)', () => {
    const mock = createMockFetch();
    const mod = new LicensingModule({} as never);
    void mod; // silence unused — we only need the type
    // Sanity check the auto-mapped surface exists on the namespaced client.
    const { norbix } = makeClient();
    const ns = (norbix.hub as unknown as Record<string, unknown>)['licensing'] as Record<
      string,
      unknown
    >;
    expect(ns).toBeDefined();
    void mock;
    expect(typeof ns['startLicenseDomainVerification']).toBe('function');
    expect(typeof ns['getLicenseDomainVerificationStatus']).toBe('function');
    expect(typeof ns['postLicenseHeartbeat']).toBe('function');
  });
  ...
  it('startLicenseDomainVerification: POST /{version}/licensing/domain-verification/start', async () => {
    ...
    const fn = (
      ...
      >
    )['licensing']!['startLicenseDomainVerification']!;
    await fn(stub);
  ...
  it('getLicenseDomainVerificationStatus: GET /{version}/licensing/domain-verification/status', async () => {
    ...
    const fn = (
      norbix.hub as unknown as Record<
        ...
        Record<string, (a?: unknown, o?: unknown) => Promise<unknown>>
      >
    )['licensing']!['getLicenseDomainVerificationStatus']!;
    await fn(stub);
    expect(mock.lastCall).toBeDefined();
  ...
  it('postLicenseHeartbeat: POST /{version}/licensing/heartbeat', async () => {
    ...
    const fn = (
      norbix.hub as unknown as Record<
        ...
        Record<string, (a?: unknown, o?: unknown) => Promise<unknown>>
      >
    )['licensing']!['postLicenseHeartbeat']!;
    await fn(stub);
    expect(mock.lastCall).toBeDefined();
    expect(mock.lastCall?.method).toBe('POST');
    expect(mock.lastCall?.url.startsWith(expected)).toBe(true);
